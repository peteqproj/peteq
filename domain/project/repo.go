...
`
var queries = []string{
	"CREATE TABLE IF NOT EXISTS projects( id text not null,userid text not null,info json not null,PRIMARY KEY (id));",
	"CREATE INDEX IF NOT EXISTS userid ON projects ( userid);",
}
...
func (r *Repo) GetById(ctx context.Context, id string) (*Project, error) {
	...
	query, _, err := goqu.From("projects").Where(e).ToSQL()
	if err != nil {
		return nil, err
	}
...
func (r *Repo) ListByUserid(ctx context.Context, userid string) ([]*Project, error) {
	...
	sql, _, err := goqu.From("projects").Where(e).ToSQL()
	if err != nil {
		return nil, err
	}
